...
void main()
{
    ...
    LightingInfo info;
    info.albedo = pow(texture(material.albedoMap, fs_in.TexCoords).rgb, vec3(2.2));
    info.metallic = texture(material.metallicMap, fs_in.TexCoords).r;
    info.roughness = texture(material.roughnessMap, fs_in.TexCoords).r;
    info.ao = 1.0; // texture(material.aoMap, fs_in.TexCoords).r;
